...
extension Modules.Wallet.Account.State.Context {
    ...
    func with(wallet: Ethereum.Wallet) -> Modules.Wallet.Account.State.Context {
        return Modules.Wallet.Account.State.Context(wallet: wallet, balance: self.balance, price: self.price)
    }

    func with(balance: BigUInt) -> Modules.Wallet.Account.State.Context {
        return Modules.Wallet.Account.State.Context(wallet: self.wallet, balance: balance, price: self.price)
    }

    func with(price: Double) -> Modules.Wallet.Account.State.Context {
        return Modules.Wallet.Account.State.Context(wallet: self.wallet, balance: self.balance, price: price)
    }
}
...
extension Modules.Wallet.Account.State {
    ...
    static func makeRequest(walletInfo: WalletProvider) -> Request {
        return Modules.Wallet.Account.State.makeRequest(when: { (state) -> Bool? in
